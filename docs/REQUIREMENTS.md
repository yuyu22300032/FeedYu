...
| Cancellation (leaving the tab) stops a scan at the next candidate, keeps queue position, and is silent — never reported as a network error, never loses the in-flight candidate. | `testCancelledRefreshStopsScanning`, `testCancellationDuringETAIsSilentAndKeepsTheCandidate`; structural: TonightView tracks every engine-touching task in `refreshTask`, cancelling the previous slot before reassigning (a revalidation runs its awaits without setting `isSearching`, so the slot may be live) |
