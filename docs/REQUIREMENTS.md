...
| **A resumed card must re-earn its button** (shipped 2026-07-15: an app suspended overnight resumed with last night's card interactive, and its slow first re-check — torn-down WebView — left a closed store's Order button live for the seconds the user needed to tap it). A revalidation that starts against an affirmation older than the 10-min open-state TTL runs as a REAL search: the engine raises `isSearching`, so the standard loading takeover pulls the card and disables rolling until the live verdict lands — the same card returns when re-affirmed, a replacement rolls when not. A fresh affirmation (casual tab switches) revalidates silently and the card never blinks. And a card the checks REJECT (persisted closed stamp, live closed verdict) is cleared before the replacement scan, so a scan that pauses at the 25-check budget — a morning where the whole neighborhood is closed — can never resurface a known-unorderable card behind the "refresh to keep looking" message. | engine: `testStaleAffirmationRevalidatesBehindTheLoadingState`, `testFreshAffirmationRevalidatesSilently`, `testStaleRevalidationStillEscalatesIntoReplacement`, `testRejectedCardNeverSurvivesAPausedReplacementScan` |
...
| **Only stores KNOWN to be ready get a card** (product call 2026-07-14, replacing the old deny-list): `storeAvailablityStatus.state` must be on the ALLOW-list (`AVAILABLE`, captured live) and not contradicted by a future `nextOpenTime`. Merchant pauses, schedule closures, courier/too-far states, UNKNOWN, unrecognized new states, and a missing state field are all "not ready" — unrecognized ones logged in DEBUG to collect vocabulary. No reopen moment → the 10-minute recheck applies (session cache and persisted stamp both). Drift risk accepted knowingly: if Uber renames AVAILABLE the tab goes visibly quiet; re-capture via the curl probe in MAINTENANCE. | `testAvailableStateIsReady`, `testAmbiguousStatesAreNotReady`, `testMissingStateFieldIsNotReady`, `testAvailableStateWithFutureReopenIsNotReady`, `testMerchantPausedStoreIsClosedWithoutReopenTime`, `testMerchantPausedKnownStoreIsSkippedEndToEnd`, `testParsesAvailabilityStateBothSpellings`, `testCachedMerchantPauseExpiresAfterTTL`, `testScheduleClosedStoreCarriesReopenTime` |
| **The open check carries the user's location** (`uev2.loc`, the same payload the search pipeline sends). A location-less getStoreV1 answers `TOO_FAR_TO_DELIVER` with a null `nextOpenTime` regardless of the real state (shipped 2026-07-14 under the old deny-list: the cookie is session-scoped and only the search pipeline set it, so a cold launch's known-store checks ran blind and a 22:00-opening store reached the lunch card; under the allow-list the same omission would hide OPEN stores instead). | `testKnownStoreOpenCheckSendsTheLocation` |
...
| After the retry an unverifiable store is SKIPPED, not shown (2026-07-14 flip of the old fail-open) — and NOTHING is cached or persisted about it, so the very next roll re-checks live; a bot-wall session shows fewer cards instead of unorderable ones. `unknown` from the search pipeline expires after 10 min in the session cache (it now hides a store, so it must not stick all session). | `testSkipsUnaffirmedStoreOnlyAfterRetryAlsoFails`, `testCachedVerdictFreshness` |
