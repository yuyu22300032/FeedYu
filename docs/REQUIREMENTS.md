...
| Cancellation (leaving the tab) stops a scan at the next candidate, keeps queue position, and is silent — never reported as a network error, never loses the in-flight candidate. | `testCancelledRefreshStopsScanning`, `testCancellationDuringETAIsSilentAndKeepsTheCandidate` |
...
| Localizer: ≤40 fetches per visit, 0.4 s apart, cached forever, cancellation never poisons the negative cache. | `MichelinNameLocalizerTests`; cancellation rule in ARCHITECTURE gotcha #13 |
