...
Paste into the version's "What's New in This Version" field per locale.
...
- A guided first launch: import your Google Maps lists step by step —
  including lists friends share with you.
- The Uber Eats tab now suggests only stores verified open and taking
  orders at this moment, checked live right before you tap Order.
  Closed stores stay hidden until they actually reopen.
- Every tab keeps its own travel budget — Tonight's drive minutes no
  longer follow you into Michelin or Uber Eats.
- Fresher on return: Michelin data and your location now update when
  you come back to the app, not just at launch.
- Faster with big collections, and a suggestion's photo always belongs
  to the restaurant on the card.
```
...
- 全新首次啟動導覽：一步一步帶你匯入 Google 地圖清單，朋友分享給你的
  清單也適用。
- Uber Eats 分頁現在只推薦「此刻確認營業中、真的接單」的店家——在你按下
  訂購前即時查核；休息中的店會隱藏到真正重新開店為止。
- 每個分頁記住自己的距離條件：「今晚」的開車分鐘數不會再跟到米其林或
  Uber Eats 分頁。
- 回到 App 即更新：米其林資料與你的位置不再只在啟動時更新。
- 大量收藏更順暢；推薦卡片的照片永遠對應正確的餐廳。
```
...
- 初回起動ガイドを追加：Google マップのリストをステップごとに取り込み。
  友達がシェアしてくれたリストにも対応。
- Uber Eats タブは「いま営業中で、実際に注文を受け付けている」と確認
  できた店だけを提案。注文ボタンを押す直前にリアルタイムでチェックし、
  休業中の店は再開まで表示しません。
- タブごとに移動範囲を記憶：「今夜」の運転分数がミシュランや Uber Eats
  タブに引き継がれることはなくなりました。
- アプリに戻るたびに更新：ミシュランのデータと現在地を起動時だけでなく
  復帰時にも更新します。
- 大きなコレクションでも軽快に。提案カードの写真は必ずその店のものに。
```
